...
public class QuestionDataDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        // Calculate rects
        var amountRect = new Rect(position.x, position.y, position.width * 0.38f, position.height);
        var unitRect = new Rect(position.x + position.width * 0.4f, position.y, position.width * 0.58f, position.height);
        //var nameRect = new Rect(position.x + position.width * 0.6f, position.y, position.width * 0.39f, position.height);
