...
public class VIDE_Editor : EditorWindow
{
    ...
    public void Save()
    {
        ...
        for (int i = 0; i < db.playerDiags.Count; i++)
        {
            ...
            // =================================================================   NAM   =================================================================
            if (db.playerDiags[i].characterData != null)
                dict.Add("pd_characterData_" + i.ToString(), AssetDatabase.GetAssetPath(db.playerDiags[i].characterData));
            else
                dict.Add("pd_characterData_" + i.ToString(), string.Empty);
            // =================================================================   NAM    =================================================================
    ...
    public void Load(bool clear)
    {
        ...
        // Ils loadent tout, bon pourquoi pas
        // ================================  NAM  ================================
        SO_CharacterData[] characterDatas = Resources.LoadAll<SO_CharacterData>("");
        List<string> characterDatasNames = new List<string>();
        foreach (SO_CharacterData t in characterDatas)
        ...
        for (int i = 0; i < pDiags; i++)
        {
            ...
            // De ce que j'ai compris, ici je load en utilisant l'identifiant spécifié dans la fonction save
            // =====================   NAM    ==================================================
            if (dict.ContainsKey("pd_characterData_" + i.ToString()))
            {
                string name = Path.GetFileNameWithoutExtension((string)dict["pd_characterData_" + i.ToString()]);
                if (characterDatasNames.Contains(name))
                    db.playerDiags[db.playerDiags.Count - 1].characterData = characterDatas[characterDatasNames.IndexOf(name)];
                else if (name != string.Empty)
                    Debug.LogError("'" + name + "' not found in any Resources folder!");
            }
            // =================================================================================
    ...
    void DrawNodeWindow(int id)
    {
        ...
        if (db.playerDiags[id].expand)
        {
            ...
            SO_CharacterData characterData = (SO_CharacterData)EditorGUILayout.ObjectField("CharacterData: ", db.playerDiags[id].characterData, typeof(SO_CharacterData), false);
            GUI.skin = pskin;
            GUI.backgroundColor = last;
            GUI.contentColor = lastc;
            if (EditorGUI.EndChangeCheck())
            {
                ...
                Undo.RecordObject(db, "Set Data");
                db.playerDiags[id].characterData = characterData;
